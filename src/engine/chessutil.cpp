...
void direct_write(index_t const pin, Bool const value) {
    // Megachess: 0xFF means "not wired". Without this the PORTD/PORTB writes
    // below land on whatever those bits are on a Mega, which is not where the
    // upstream Uno pin numbers point.
    if (0xFF == (uint8_t) pin) { return; }
