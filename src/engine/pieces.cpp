...
index_t add_pawn_moves(piece_gen_t &gen) {
    ...
    // Check 1 row ahead
    index_t const one_ahead = check_fwd(gen, to_col, to_row);
    count += one_ahead;

    if (timeout()) { return count; }

    // Check 2 rows ahead - but only if the square directly in front was
    // clear. Upstream tried the two-square move regardless, so a pawn could
    // jump over a piece standing right in front of it.
    // (Megachess fix.)
    if (one_ahead && !hasMoved(board.get(gen.move.from))) {
